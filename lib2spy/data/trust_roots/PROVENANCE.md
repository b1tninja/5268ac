...
> See [`lib2spy/pkgstream_verify.py`](../pkgstream_verify.py) — `ProvenanceWarning`
...
1. Mount or extract a target firmware's rootfs (e.g. via `binwalk`, `unsquashfs`).
...
5. **Bump the `BUNDLE_VERSION` constant in [`lib2spy/pkgstream_verify.py`](../pkgstream_verify.py)** so the runtime warning re-fires on first use.
